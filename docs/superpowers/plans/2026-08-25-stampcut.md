...
- 줌 아웃 시 `pad` 채움색은 스펙의 "black" 대신 `Settings.background_color`(기본 `#000000`)를 쓴다 — 미리보기의 정방형 배경과 캔버스 색이 항상 일치하도록. 기본값에서는 스펙과 동일.
- 영상 정보 조회에서 일부 영상이 없을 때(비공개·삭제·오타) 스펙 §7.1의 "빈 items → VideoNotFound"보다 §10의 "그 영상만 오류로 표시, 나머지 진행"을 따른다: `fetch_video_infos(urls, strict=False)`는 찾은 영상만 돌려주고, `analyze`는 빠진 URL을 `Project.warnings`에 담아 GUI가 안내한다. 전부 없을 때만 `VideoNotFound`.
- 자막 위치는 스펙 §3.3의 `y=1560, line_spacing 8` 대신 `caption_y=1552, line_spacing=0`을 쓴다 — 2줄 자막의 아래끝(1552+120+테두리 4 = 1676)이 릴스 UI 구역(y≥1680)을 넘지 않도록. 시간(36px, y=1510)은 그대로.
- 렌더는 concat 결과를 작업 폴더의 임시 파일에 쓴 뒤 `os.replace`로 출력 경로에 옮긴다(취소·실패 시 출력 폴더에 깨진 파일이 남지 않음). 성공한 렌더는 자신의 작업 폴더뿐 아니라 남아 있는 이전 작업 폴더도 정리한다 (스펙 §8.5 "다음 성공 시 정리").
- yt-dlp format 문자열은 스펙의 `bv*[height<=360]+ba/b[height<=360]` 뒤에 `/bv*+ba/b` 폴백을 붙인다 (해당 해상도 이하 렌디션이 전혀 없는 드문 경우에도 실패 대신 받기 위해). 최종 format도 같은 방식.
- drawtext의 `textfile=`/`fontfile=` 경로는 `'E\:/path'` 형태(작은따옴표 + `\:`)로 넘긴다. ffmpeg 9.0으로 실측 확인(2026-08-25): 필터그래프 1단계 파싱이 따옴표를 벗기며 `\:`를 보존하고, 2단계 옵션 파싱이 `\:`를 `:`로 되돌린다. 따옴표 없는 형태는 실패한다.
